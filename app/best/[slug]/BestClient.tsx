...
const RANK_STYLES = [
  ...
  { badge: "🥈", border: "border-slate-200", bg: "bg-white/70",  label: "text-slate-500" },
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-4">
        {data.picks.map((pick, i) => {
          const style = RANK_STYLES[i] ?? { badge: `#${i + 1}`, border: "border-slate-200", bg: "bg-white/70", label: "text-slate-500" };
          return (
            <div
              key={pick.name}
            ...
            >
              ...
              <div className="flex items-start gap-4">
                ...
                <Link
                  ...
                  className="hidden sm:inline-flex shrink-0 items-center gap-1 rounded-lg border border-slate-200 bg-white/80 px-3 py-1.5 text-xs text-slate-400 hover:border-violet-200 hover:text-violet-700 transition-all"
                >
                  Compare →
                </Link>
              </div>
              ...
              {/* Best for */}
              <div className="rounded-xl bg-white/80 border border-slate-200 px-4 py-2.5 text-sm">
          ...
          {items.slice(0, 6).map((item, i) =>
            items.slice(i + 1, 7).map((item2) => (
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white/80 px-3 py-1.5 text-xs text-slate-400 hover:border-violet-200 hover:text-violet-700 transition-all"
              >
      ...
      {data.faqs && data.faqs.length > 0 && (
        <>
          ...
          />
          <div className="rounded-2xl border border-slate-200 bg-white/70 px-6 py-2">
