...
const CAT_COLORS: Record<string, string> = {
  ...
  "Laptops & Hardware": "bg-slate-500/20 text-slate-300 border-slate-500/30",
  ...
  "Website Builders":   "bg-emerald-500/20 text-emerald-300 border-emerald-500/30",
  "By Region":          "bg-teal-500/20 text-teal-300 border-teal-500/30",
};
...
function catClass(cat: string, active = false) {
  ...
  return map[cat] ?? (active ? "bg-slate-600 text-white border-slate-600" : "bg-slate-700/50 text-slate-300 border-slate-700");
}

function TopicCard({ topic }: { topic: BestTopic }) {
  return (
    <Link href={`/best/${topic.slug}`} className="group flex flex-col gap-3 rounded-xl border border-slate-800 bg-slate-900/60 p-5 transition-all hover:border-violet-500/40 hover:shadow-lg hover:shadow-black/30 hover:-translate-y-0.5">
      ...
      <div className="flex-1">
        <h3 className="font-semibold text-white group-hover:text-violet-300 transition-colors text-sm leading-snug">{topic.title}</h3>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-violet-400 font-medium group-hover:translate-x-0.5 transition-transform">See rankings →</span>
      </div>
    </Link>
  );
}
  ...
  return (
    <div>
      ...
      <div ref={containerRef} className="relative mb-6">
        <div className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 transition-all bg-slate-900 ${searchOpen || query ? "border-violet-500 ring-2 ring-violet-500/20" : "border-slate-800 hover:border-slate-700"}`}>
          ...
          />
          {query && <button onClick={closeSearch} className="text-slate-500 hover:text-slate-300 cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>}
        </div>
        {searchOpen && query.trim().length > 0 && (
          <div className="absolute left-0 right-0 top-full z-50 mt-1.5 rounded-xl border border-slate-800 bg-slate-900 shadow-xl shadow-black/40 overflow-hidden">
            {searchResults.length === 0
              ? <p className="px-4 py-3 text-sm text-slate-500">No guides found for &ldquo;{query}&rdquo;</p>
              : <ul>{searchResults.map((t) => (
                  <li key={t.slug}>
                    <Link href={`/best/${t.slug}`} onClick={closeSearch} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-800 transition-colors">
      ...
      {showFeatured && featured && (
        <Link href={`/best/${featured.slug}`} className="group mb-10 flex flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60 transition-all hover:border-violet-500/40 hover:shadow-lg hover:shadow-black/30 sm:flex-row">
          ...
          <div className="flex flex-1 flex-col justify-center p-6">
            ...
            <h2 className="text-xl font-bold text-white group-hover:text-violet-300 transition-colors">{featured.title}</h2>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {featured.items.slice(0, 5).map((item) => (
                <span key={item} className="rounded-full bg-slate-800 px-2.5 py-0.5 text-xs text-slate-400 border border-slate-700">{item}</span>
              ))}
            </div>
          </div>
        </Link>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => handleCategory(null)} className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium transition-all cursor-pointer ${active === null && !searchOpen ? "bg-white text-slate-900 border-white" : "bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-700 hover:text-slate-300"}`}>All</button>
      ...
      {!searchOpen && totalPages > 1 && (
        <div className="mt-10 flex items-center justify-center gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-800 bg-slate-900 text-slate-400 transition-all hover:border-violet-500/40 hover:text-violet-400 disabled:opacity-30 disabled:cursor-not-allowed">‹</button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => {
            ...
            return <button key={p} onClick={() => setPage(p)} className={`flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-medium transition-all ${isActive ? "bg-violet-600 border-violet-600 text-white" : "border-slate-800 bg-slate-900 text-slate-400 hover:border-violet-500/40 hover:text-violet-400"}`}>{p}</button>;
          })}
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-800 bg-slate-900 text-slate-400 transition-all hover:border-violet-500/40 hover:text-violet-400 disabled:opacity-30 disabled:cursor-not-allowed">›</button>
        </div>
      )}
    </div>
  );
