...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Smartphones:   "bg-pink-500/20 text-pink-300 border-pink-500/30",
  Entertainment: "bg-teal-500/20 text-teal-300 border-teal-500/30",
  Apps:          "bg-teal-500/20 text-teal-300 border-teal-500/30",
};
    ...
    <div>
      ...
      <div ref={containerRef} className="relative mb-6">
        <div className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 transition-all bg-slate-900 ${searchOpen || query ? "border-violet-500 ring-2 ring-violet-500/20" : "border-slate-800 hover:border-slate-700"}`}>
          ...
          />
          {query && (
            <button onClick={closeSearch} className="text-slate-500 hover:text-slate-300 cursor-pointer">
          ...
          <div className="absolute left-0 right-0 top-full z-50 mt-1.5 rounded-xl border border-slate-800 bg-slate-900 shadow-xl shadow-black/40 overflow-hidden">
            {searchResults.length === 0
              ...
              : <ul>{searchResults.map((a) => (
                  <li key={a.slug}>
                    <Link href={`/articles/${a.slug}`} onClick={closeSearch} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-800 transition-colors">
      ...
      {/* Featured article */}
      {showFeatured && (
        <Link href={`/articles/${featured.slug}`} className="group mb-10 flex flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60 transition-all hover:border-violet-500/40 hover:shadow-lg hover:shadow-black/30 sm:flex-row">
          ...
          <div className="flex flex-1 flex-col justify-center p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {featured.tags.slice(0, 3).map((tag) => (
                <span key={tag} className="rounded-full bg-slate-800 px-2.5 py-0.5 text-xs text-slate-400 border border-slate-700">#{tag}</span>
              ))}
            </div>
          </div>
        </Link>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => handleCategory(null)} className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium transition-all cursor-pointer ${active === null ? "bg-white text-slate-900 border-white" : "bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-700 hover:text-slate-300"}`}>All</button>
        {categories.map((cat) => (
...
function ArticleCard({ article, categoryClass }: { article: Article; categoryClass: (c: string) => string }) {
  return (
    <Link href={`/articles/${article.slug}`} className="group flex flex-col overflow-hidden rounded-xl border border-slate-800 bg-slate-900/60 transition-all hover:border-violet-500/40 hover:shadow-lg hover:shadow-black/30 hover:-translate-y-0.5">
      ...
      <div className="flex flex-1 flex-col p-4">
        <h3 className="flex-1 text-sm font-semibold text-white group-hover:text-violet-300 transition-colors leading-snug">{article.title}</h3>
